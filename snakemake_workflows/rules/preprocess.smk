...
from cosap._pipeline_config import MDUPKeys,BaseRecalibratorKeys,TrimmingKeys,PipelineKeys
from collections import defaultdict


rule fastp_trim:
    output:
        fastq_outputs=expand(config[PipelineKeys.TRIM][TrimmingKeys.SNAKEMAKE_OUTPUT], pair=["1","2"])
    run:
...
rule mark_dup:
    input:
        bam=lambda wildcards: config[PipelineKeys.MDUP][wildcards.identification][MDUPKeys.INPUT]
    output:
...
rule gatk_base_cal:
    input:
        bam=lambda wildcards: config[PipelineKeys.CALIBRATE][wildcards.identification][BaseRecalibratorKeys.INPUT]
    output:
        calibrated_bam=config[PipelineKeys.CALIBRATE][BaseRecalibratorKeys.SNAKEMAKE_OUTPUT]
    run:
        duplicate_remover = PreprocessorFactory.create(
