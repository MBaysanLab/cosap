...
class DownloadFile:
    def __init__(self, url, filename=None, size=None, md5=None):
        ...
        self.size = (
            size if size else int(requests.get(url).headers.get("content-length"))
        )

        self.md5 = md5

        self.download_path = os.path.join(AppConfig.LIBRARY_PATH, self.filename)
        ...
        with requests.get(self.url, stream=True, headers=headers) as response:
            with open(self.download_path, "ab") as output_file:
                for i, chunk in enumerate(response.iter_content(chunk_size=chunk_size)):
                    ...
                    _reporthook(
                        self.downloaded_size + ((i + 1) * chunk_size), self.size
                    )
...
def call_download_files():
    # TODO: Check available space
    print(
        "To change the download location, set the environment variable ` COSAP_LIBRARY_PATH ` to the desired path."
    )
    ...
    file_info_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "files.json"
    )

    file_info_list = json.load(open(file_info_path))["files"]
    ...
    incomplete_files = [
        ...
        if file.status
        not in (DownloadFileStatus.present_fully, DownloadFileStatus.downloaded_fully)
    ]
    ...
    if already_present_count:
        print(
            f"{already_present_count}/{total_files_count} files were already present."
        )

    if downloaded_count:
        print(
            f"{downloaded_count}/{total_files_count} files were downloaded successfully."
        )

    if download_blocked_count:
        print(
            f"{download_blocked_count}/{total_files_count} files could not be downloaded:"
        )
        [
            print("-", filename)
            for filename in statuses.keys()
            if statuses[filename] == DownloadFileStatus.download_blocked
        ]

    if already_present_count + downloaded_count < total_files_count:
        print(
            f"{total_files_count - (already_present_count + downloaded_count)}/{total_files_count} are missing or incomplete."
        )
    else:
